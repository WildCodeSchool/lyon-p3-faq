...
class Question extends DB {
  ...
  getIds(ids) {
    const query =
      "SELECT question.titre, question.contenu, question.id, user.name AS asker, COUNT(upvote.id_reponse) AS voteup FROM question JOIN reponse ON question.id = reponse.question_id LEFT JOIN upvote ON reponse.question_id=upvote.id_reponse JOIN user ON user.id = question.created_by WHERE question.disabled_at IS NULL AND question.id IN ? GROUP BY question.titre, question.contenu, question.id, reponse.created_by";
    return this.q(query, [[ids]]);
  }
  ...
  getAll() {
    const results = "SELECT question.id, question.titre, question.contenu, reponse.contenu AS reponse FROM question JOIN reponse ON question.id=reponse.question_id"
