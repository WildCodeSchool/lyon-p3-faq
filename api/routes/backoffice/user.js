...
router.get("/", (req, res) => {
  db.query(
    "SELECT name,mail,pass,ip_adress,role_id FROM user",
    (err, result) => {
      if (err) {
      ...
      (err, result) => {
        if (err) {
          ...
          if (result[0].count > 0) {
            ...
            db.query(
              "SELECT name,mail,ip_adress,role_id FROM user WHERE id= ?",
              idUser,
              (err, result) => {
                if (err) {
                  res.status(500).send({ error: err });
...
router.post("/", (req, res) => {
  ...
  db.query(
    "INSERT INTO user (name,mail,role_id,pass,ip_adress) VALUES ? ",
    [fields],
    (err, result) => {
      if (err) {
        res.status(500).send({ error: err });
